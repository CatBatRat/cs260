...
int main() {
  ...
  std::cout << "Should be 30 and is " << pile.largest() << std::endl
            << std::endl;

  // now add more values, causing doubling
  std::cout << "Now adding 40, 60, 25, 50, 35, should have doubled size"
            << std::endl
            << std::endl;
  ...
  std::cout << "Removing from largest to smallest" << std::endl
            << "Expected results: 60 50 40 35 30 25 20 15 10 5" << std::endl;
  ...
  int pq_values[PQ_NUM_VALUES] = {1,  13, 21, 3,  5,  7,  9, 11,
                                  15, 23, 17, 19, 25, 27, 29};

  PriorityQueue theQueue(PQ_NUM_VALUES);

  // load the heap with values
  std::cout << "Loading the queue with first 15 odd numbers in scrambled order"
            << std::endl
            << std::endl;
  for (int i = 0; i < PQ_NUM_VALUES; i++)
    theQueue.insert(pq_values[i]);

  std::cout << "Displaying largest, should be 29 and it is " << theQueue.peek()
            << std::endl
            << std::endl;

  // remove values, should be in ascending order
  std::cout << "Now removing values, should be in descending order"
            << std::endl;
